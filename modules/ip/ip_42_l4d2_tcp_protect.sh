...
ip_42_l4d2_tcp_protect_validate() {
    ...
    case "${ENABLE_L4D2_TCP_PROTECT:-false}" in
        true|false) ;;
        *)
            echo "ERROR: ip_l4d2_tcp_protect: ENABLE_L4D2_TCP_PROTECT must be true or false"
            return 2
            ;;
    esac
}
...
ip_42_l4d2_tcp_protect_apply() {
    ...
    if [ "${ENABLE_L4D2_TCP_PROTECT:-false}" = "true" ]; then
        echo "L4D2 TCP protection ENABLED: blocking RCON/game TCP abuse"
        ...
        echo "L4D2 TCP protection DISABLED"
        if [ -n "${L4D2_TCP_PROTECTION:-}" ]; then
            iptables -A INPUT -p tcp -m multiport --dports "$L4D2_TCP_PROTECTION" -j TCPfilter
            if [ "$TYPECHAIN" -eq 1 ] || [ "$TYPECHAIN" -eq 2 ]; then
                iptables -A DOCKER-USER -p tcp -m multiport --dports "$L4D2_TCP_PROTECTION" -j TCPfilter
            fi
        fi
    fi
}
